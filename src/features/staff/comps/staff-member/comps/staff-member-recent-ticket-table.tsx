...
import { StaffMemberRecentTicketTableRow } from './staff-member-recent-ticket-table-row';
...
export default function StaffMemberRecentTicketTable({ invoices }: Props) {
    return (
        <Table>
            <TableHeader>
            ...
            <TableBody>
                ...
                {isEmpty(invoices) ? (
                    ...
                    invoices.map((invoice) => <StaffMemberRecentTicketTableRow key={invoice.id} invoice={invoice} />)
                )}
            </TableBody>
        </Table>
    );
