...
import { ClientRecentTicketTableRow } from './client-recent-ticket-table-row';
...
export default function ClientRecentTicketTable({ invoices }: Props) {
    return (
        <Table>
            <TableHeader>
            ...
            <TableBody>
                ...
                {isEmpty(invoices) ? (
                    ...
                    invoices.map((invoice) => <ClientRecentTicketTableRow key={invoice.id} invoice={invoice} />)
                )}
            </TableBody>
        </Table>
    );
