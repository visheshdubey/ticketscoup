...
import { ClientsPageTableRow } from './clients-page-table-row';

type Props = {
    clients: Client[];
};

export function ClientsPageTable({ clients }: Props) {
    return (
        <Table>
            <TableHeader>
            ...
            <TableBody>
                ...
                {isEmpty(clients) ? (
                    ...
                    clients.map((client) => <ClientsPageTableRow key={client.id} client={client} />)
                )}
            </TableBody>
        </Table>
    );
