...
export function NavMain({
    ...
    return (
        <SidebarGroup className={`${open ? 'pl-[25px] pr-[22px]' : ''} font-satoshi`}>
            <SidebarGroupLabel className="uppercase font-bold text-xs text-[#A09B96] leading-[17px]">
                {groupLabel}
            </SidebarGroupLabel>
            <SidebarMenu className="pt-4 gap-1.5">
                {items.map((item) =>
                    item.items && item.items.length > 0 && item.requiredRoleAny?.includes(userRole) ? (
                        <Collapsible
                            key={item.title}
                            asChild
                                    ...
                                    <SidebarMenuButton tooltip={item.title}>
                                        <div className="flex [&>svg]:size-4 [&>svg]:shrink-0 space-x-4 items-center">
                                            <div className="flex -ml-0.5">
                                                {item.icon && <item.icon strokeWidth={1.75} width={20} height={20} />}
                                            </div>
                        ...
                        item.requiredRoleAny?.includes(userRole) && (
                            <SidebarMenuItem key={item.title}>
                                <SidebarMenuButton asChild tooltip={item.title} className="!p-[0.4rem]">
                                    <a
                                        href={item.url}
